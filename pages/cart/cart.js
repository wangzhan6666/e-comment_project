 ...
 * 1 获取用户的收货地址
 *    1 绑定点击事件
 *    2 调用襄城县内置  api  获取用户的收货地址   wx.chooseAddress
 * 
 * 
 *    2 获取  用户  对襄城县 所授予  获取地址的   权限 状态  scope
 *      1 假设  用户 点击获取收货地址的提示框  确定   scope.address
 *        scope  值  为true   直接调用   获取收货地址
 *      2 假设 用户 从来诶呦调用过  收货地址的  api
 *        scope 值   为undefined  直接调用   获取收货地址
 ...
 * 
 * 3 onShow
 *    0 回到了商品详情页面  第一次添加商品的时候  手动添加了页面属性
 *      1 num=1;
 *      2 checked=true;
 *    1 获取缓存中的购物车数组
 *    2 把购物车数据  填充到购物车数组
 ...
 *    4 遍历购物车数组  让里面  商品  选中状态allChecked   改变而改变
 *    5 把购物车数组  和 allChecked  冲农信设置会data  把购物车重新设置会缓存中
 * 
 * 
 * 8 商品数量的编辑
...
Page({
  ...
  onShow(){
    ...
    // 1 计算全选
    // every  数组方法   会遍历 会接收一个回调函数  name  每一个回调函数都返回true  那么  every方法的返回值为true
